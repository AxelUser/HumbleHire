<script lang="ts">
	...
	let { cv, master, isLast, onDelete, onSync }: Props = $props();

	const updatesAvailable = $derived(hasUpdatesAvailable(master, cv));
	const target = $derived(cv.company ?? cv.name);
	const subtitle = $derived(cv.company ? cv.name : '');
	...
	>
		...
		<div class="flex min-w-0 flex-col gap-0.5">
			<div class="flex flex-wrap items-baseline gap-x-1 text-sm">
				<strong class="font-bold">{target}</strong>
				{#if subtitle}
					<span class="text-muted-foreground font-medium">· {subtitle}</span>
				{/if}
			</div>
			<div
