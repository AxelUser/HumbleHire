...
`window.__hhTest` is attached only when `import.meta.env.VITE_E2E` is set, so it is absent from both normal dev and normal production bundles. The e2e run builds and previews with `VITE_E2E=true`. A build-time flag, rather than a `dev` check, lets the suite run against a real production preview build while still reaching the seed bridge — closer to what ships, and decoupled from the `dev`-gated Dev Toolbox, which stays a manual affordance.
